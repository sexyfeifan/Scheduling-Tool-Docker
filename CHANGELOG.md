...
## v3.31 (2026-07-07) - 里程碑版本：移动端全面重构 + 只读子页面 + 类型颜色系统

### 📱 移动端响应式重构（v3.04~v3.31）
- **设备检测**: UA + `maxTouchPoints > 0` + `ontouchstart` 三重检测，覆盖 iPhone/iPad/Android/小米 Pad
- **日/周视图统合**: 移动端日视图和周视图共享日期选择器 + 详情区显示方式
- **日期选择器**: 7天横向排列（周一~周日 + 日期数字 + 项目数），可滚动，选中态高亮
- **详情区**: 点击日期后下方大面积显示当日所有项目卡片（含删除/复制/新增按钮）
- **自动选中**: 进入页面自动选中今天并显示当日项目
- **周切换同步**: 左右切换周数时，日期选择器和详情区自动跟随更新
- **底部导航栏**: 日/周/月/人/导出 5 个按钮，触屏设备始终显示
- **工具栏保留**: 所有触屏设备完整显示桌面端工具栏（新增/导出/管理等）
- **月视图**: 日历 grid 横向滚动
- **人员视图**: 矩阵表格横向滚动

### 🔗 只读分享子页面
- **访问路径**: `/canbox`（需在管理后台启用）
- **管理员控制**: 设置中可开关 + 自定义路径
- **UI**: 底部导航栏 + 完整工具栏 + 日期选择器 + 详情区
- **调试工具**: URL 带 `?debug=1` 加载 eruda 移动端调试面板

### 🐛 导出修复
- **颜色修复**: 移除 `getComputedStyle` 覆盖（detach 元素返回默认值），clone 的 inline style 保留完整
- **颜色烘焙**: clone append 后遍历子元素，将 `backgroundColor`/`color`/`borderColor` 的 computedStyle 写入 inline style
- **移动端 scale**: 从 `2` 降为 `1.5`，减少内存压力

### 🏗️ 基础设施
- **SW 缓存**: 策略改为网络优先（确保更新及时生效），缓存名随版本更新
