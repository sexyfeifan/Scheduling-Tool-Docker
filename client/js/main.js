...
const directorOptionsDiv = document.getElementById('project-director-options');
const photographerOptionsDiv = document.getElementById('project-photographer-options');
const productionOptionsDiv = document.getElementById('project-production-options');
const operationalOptionsDiv = document.getElementById('project-operational-options');
const rdOptionsDiv = document.getElementById('project-rd-options');
const audioOptionsDiv = document.getElementById('project-audio-options');
...
function applyTemplateToForm(template) {
    ...
    setCheckedValues(directorOptionsDiv, defaults.director || '');
    setCheckedValues(photographerOptionsDiv, defaults.photographer || '');
    setCheckedValues(productionOptionsDiv, defaults.production || '');
    setCheckedValues(rdOptionsDiv, defaults.rd || '');
    setCheckedValues(operationalOptionsDiv, defaults.operational || '');
    setCheckedValues(audioOptionsDiv, defaults.audio || '');
}
...
async function saveTemplateFromCurrentForm() {
    ...
    const payload = {
        ...
        defaults: {
            ...
            director: getCheckedValues(directorOptionsDiv),
            photographer: getCheckedValues(photographerOptionsDiv),
            production: getCheckedValues(productionOptionsDiv),
            rd: getCheckedValues(rdOptionsDiv),
            operational: getCheckedValues(operationalOptionsDiv),
            audio: getCheckedValues(audioOptionsDiv),
...
function createProjectCard(project, dateStr, projectIndex) {
    ...
    if (project.director || project.photographer || project.production || project.rd || project.operational || project.audio || project.startTime) {
        ...
        if (project.director) {
            staffInfo += `<span class="staff-role director">导演：${escapeHtml(project.director)}</span>`;
        }
        if (project.photographer) {
            staffInfo += `<span class="staff-role photographer">摄影：${escapeHtml(project.photographer)}</span>`;
        }
        if (project.production) {
            staffInfo += `<span class="staff-role production">制片：${escapeHtml(project.production)}</span>`;
        }
        if (project.rd) {
            staffInfo += `<span class="staff-role rd">研发：${escapeHtml(project.rd)}</span>`;
        }
        if (project.operational) {
            staffInfo += `<span class="staff-role operational">运营：${escapeHtml(project.operational)}</span>`;
        }
        if (project.audio) {
            staffInfo += `<span class="staff-role audio">录音：${escapeHtml(project.audio)}</span>`;
        }
        staffInfo += '</div>';
    }
...
function editProject(dateStr, projectIndex) {
    ...
    // 设置各分类（多选）
    setCheckedValues(directorOptionsDiv, project.director || '');
    setCheckedValues(photographerOptionsDiv, project.photographer || '');
    setCheckedValues(productionOptionsDiv, project.production || '');
    setCheckedValues(rdOptionsDiv, project.rd || '');
    setCheckedValues(operationalOptionsDiv, project.operational || '');
    setCheckedValues(audioOptionsDiv, project.audio || '');
...
function updateProjectFormOptions() {
    ...
    renderCheckboxGroup(directorOptionsDiv, directors);
    renderCheckboxGroup(photographerOptionsDiv, photographers);
    renderCheckboxGroup(productionOptionsDiv, productions);
    renderCheckboxGroup(operationalOptionsDiv, operationals);
    renderCheckboxGroup(rdOptionsDiv, rds);
    renderCheckboxGroup(audioOptionsDiv, audios);
}
    ...
    if (projects.length === 0) {
        ...
        body.innerHTML = projects.map((p, i) => {
            const meta = [
                ...
                p.director ? `导演: ${escapeHtml(p.director)}` : '',
                p.photographer ? `摄影: ${escapeHtml(p.photographer)}` : '',
                p.production ? `制片: ${escapeHtml(p.production)}` : '',
                p.type ? `类型: ${escapeHtml(p.type)}` : ''
            ].filter(Boolean);
