...
export function createScheduleCardModule(ctx) {
    ...
    function createProjectCard(project, dateStr, projectIndex) {
        ...
        const locationCat = roleCategories.find(c => c.key === 'location');
        const otherCats = roleCategories.filter(c => c.key !== 'location');

        let metaHtml = '';
        if (project.startTime) {
            metaHtml += `<span class="project-meta">⏰ ${escapeHtml(project.startTime)}</span>`;
        }
        if (project.location) {
            metaHtml += `<span class="project-meta">📍 ${escapeHtml(project.location)}</span>`;
        }
        otherCats.forEach(cat => {
            const val = project[cat.key] || (project.customFields && project.customFields[cat.key]);
            if (val) {
                metaHtml += `<span class="project-meta">${escapeHtml(cat.label)}: ${escapeHtml(val)}</span>`;
            }
        });
        if (project.isAdvertiser && project.advertiserNo) {
            metaHtml += `<span class="project-advertiser-no">商单 #${escapeHtml(project.advertiserNo)}</span>`;
        }

        card.innerHTML = `
            <div class="project-card-header">
                <span class="project-name">${escapeHtml(project.name || '')}</span>
                <span class="project-type-badge ${typeClass}">${escapeHtml(project.type || '')}</span>
            </div>
        ...
        card.addEventListener('dragstart', (e) => ctx.handleDragStart(e));
        card.addEventListener('dragend', ctx.handleDragEnd);

        card.querySelector('.edit-btn').addEventListener('click', (e) => {
            e.stopPropagation();
            ctx.editProject(dateStr, projectIndex);
        });

        card.querySelector('.copy-btn').addEventListener('click', (e) => {
            e.stopPropagation();
            ctx.showCopyModal(dateStr, projectIndex);
        });

        card.querySelector('.delete-btn').addEventListener('click', (e) => {
            e.stopPropagation();
            ctx.deleteProject(dateStr, projectIndex);
        });
